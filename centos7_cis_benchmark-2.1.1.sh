...
############################################
  ...
  ###################################
    ...
    ############################################
      ...
      msg "useradd -D | grep INACTIVE =>$(useradd -D | grep INACTIVE | cut -d= -f2)"
      ...
      msg "Verify no user has inactive password <=30 days @ $(egrep ^[^:]+:[^\!*] /etc/shadow | cut -d: -f1 | xargs -n1 chage --list | grep warning | uniq | cut -d: -f2)"
